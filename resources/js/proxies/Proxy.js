...
class BaseProxy {
	 ...
	 *
	 * @param {Object} parameters The given parzzameters.
	 *
	...
	async submit (requestType, url, data = null) {
		try {
			...
			let response = await Axios[requestType](url + this.getParameterString(), data);
			// if (process.env.NODE_ENV === "production") return response.data;
